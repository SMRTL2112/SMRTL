...
inner join edd_mapper ed2 on ed2.EDD_VALUE = substr(extractvalue(xml_doc,'*/TestingAuthority'),1,30) and ed2.code_type = 'B_PROFILE'
) loop
...
and code_type = 'B_PROFILE'
and edd_value = r_rec.code
and qual_1 = decode(r_rec.qual_1,'CBC','EDTA','HGH','SST','HGH2','SST');

if v_count = 0 then
  select ct$report_Datamap_seq.nextval
  into v_seq from dual;

  insert into ct$report_datamaps (id,map_id,code_type,edd_value,horizon_value,edit_state,QUAL_1)
  values (-v_seq, 2044, 'B_PROFILE', r_rec.code, r_rec.profile, 'N',decode(r_rec.qual_1,'CBC','EDTA','HGH','SST','HGH2','SST'));
end if;
end loop;
end;
